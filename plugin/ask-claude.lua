...
local function ask_claude(prompt)
...
local function ask_claude_explain(context)
	...
	if mode == "v" or mode == "V" or mode == "\22" then
		-- HACK: Exit visual mode, otherwise we get the previous selection
...
local function ask_claude_diagnostics(context)
...
vim.api.nvim_create_user_command("AskClaude", toggle_claude, {})
vim.api.nvim_create_user_command("AskClaudeExplain", ask_claude_explain, { range = true })
vim.api.nvim_create_user_command("AskClaudeDiagnostics", ask_claude_diagnostics, { range = true })
